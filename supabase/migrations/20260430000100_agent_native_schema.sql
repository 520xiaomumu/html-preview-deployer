...
ON agent_keys (owner_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_sites_owner_updated
ON sites (owner_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_sv_site_ver
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'fk_sites_current_version'
  ) THEN
    ALTER TABLE sites
